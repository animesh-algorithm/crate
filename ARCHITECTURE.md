...
Cloud persistence uses a small owner-only manifest plus two fixed Firestore snapshot namespaces, `snapshot-0` and `snapshot-1`, each with at most 128 fixed numeric chunk paths. Every staged parent and chunk carries its revision and a random writer ID; the manifest carries the matching snapshot and writer IDs. Rules permit writes only to the inactive next slot, and parent overwrite arbitrates concurrent writers before their chunks or manifest can be accepted. Failed and interrupted stages may leave stale documents, but they cannot allocate paths outside the finite two-slot namespace. This avoids Firestore's per-document size limit without silently overwriting another device. Firestore rules, not browser checks, are the authority for owner isolation and the closed release gate.
...
- A manifest holds `{revision,snapshotId,snapshotWriterId}` and points to the matching parent in one of two alternating fixed slots. A client preflights the current revision, overwrites only the inactive next slot with a fresh writer ID, writes fenced chunks, then advances the manifest in a Firestore transaction when the expected revision and staged parent still match. Wrong revisions raise `REVISION_CONFLICT`. Reads reject incomplete or mixed-writer snapshots.
...
- Account deletion first creates an immutable content-free deletion guard for the Firebase UID. Rules then deny every create or update from other tabs while allowing cleanup of both fixed slots and legacy snapshots; the client removes snapshots and the manifest before deleting the active Firebase Authentication account. The guard remains so the same UID cannot silently rehydrate. Recent login may be required by Firebase.
...
## Public and library route boundary

`src/App.tsx` is the routing boundary. It loads the marketing route and the `/app/*` library shell as separate lazy chunks. The provider may continue opening the owner-keyed library while `/` renders immediately; public storytelling never waits on IndexedDB or Firebase. `motion/react`, marketing motion policy, and marketing CSS are imported only by the public route. The existing parser, library schema, search, organization, IndexedDB, and Firebase modules are unchanged.

Bundled marketing media is stored in responsive WebP pairs under `public/marketing`. These files are fictional and local; below-fold images use native lazy loading. The public search and drag demonstrations use component state only and have no storage or synchronization access. Application navigation, loading, sample state, global errors, import dialog state, and save status remain in `LibraryApp`.

`LibraryApp` is also the new-account gate. A signed-out visitor with no stored local library sees Google entry and the isolated sample option; an existing `local` IndexedDB record continues to open and exposes an explicit backup/sign-in/restore migration path. After authentication, the client reads `/config/release` before offering the first import. Firestore rules remain the authority when the first snapshot is written.

Suggested collection routes read the existing owner-keyed `ProposalDraft`; no Library schema field or cloud payload is added. `/app/suggested/:id` resolves a group by its stable proposal identity. Global search applies proposal membership as a post-authorization filter through `scope`, while the existing exact/vector ranking and `collection` filter remain unchanged.
