...
Before opening admissions, verify the `/app` entry screen completes real Google popup and redirect sign-in, reads `/config/release`, blocks first import while `accepting` is false, and permits it only after the live rules checklist passes. Recheck the Instagram Accounts Center labels against the current app and official help surface; update the guide if Meta has renamed Export your information, Saved items and collections, Export to device, All time, or JSON. Verify a grandfathered local library can still be opened and exported without signing in, and that sign-in does not upload it.
