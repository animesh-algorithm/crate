...
No Cloud Functions, paid billing, Firebase hosting, service-account credential, or server-side personal-data processor is introduced. Account deletion retains one immutable content-free `{blocked:true}` guard keyed only by the former Firebase UID, deletes the caller's Firestore snapshots and manifest, then deletes that same active Firebase Authentication account; recent-login requirements may require another sign-in. Rule deployment and live authenticated create/read/update/delete verification remain release gates.
...
## ADR-018: Bounded immutable cloud snapshot window

Accepted security correction, 2026-09-19. Firestore uses exactly two reusable snapshot parents, `snapshot-0` and `snapshot-1`, with numeric chunk IDs bounded from 0 through 127. Snapshot parents and chunks carry a revision and random writer ID; the manifest carries the matching snapshot and writer IDs. Rules allow create or overwrite only in the inactive slot for the exact next revision. A competing writer can replace an inactive staged parent, but its writer ID then fences the losing writer's chunks and manifest update. Active parent and chunk mutation or deletion is denied during normal operation. Reads accept only a complete contiguous chunk set matching the manifest's revision and writer. Interrupted writes can leave stale data only inside the two finite namespaces, preventing unbounded orphan paths. Existing random-ID manifests remain readable and migrate on their next successful write; account deletion also enumerates and removes legacy parents. No paid service or server runtime is introduced.

## ADR-019: Immutable account-deletion guard

Accepted security correction, 2026-09-19. Before deleting any library document, the authenticated client creates `/users/{uid}/library/deletion` with exactly `{blocked:true}`. Only that owner can create or read it; nobody can update or delete it. Every manifest, snapshot, and chunk create or update requires the guard to be absent, while the guard permits deletion of active and inactive data. This closes the cross-tab race where manifest removal previously made the account appear fresh. Cleanup retries observe the existing guard and continue idempotently. The guard intentionally remains after Firebase Authentication deletion so the same UID cannot rehydrate data. It contains no private library data, identity attributes, timestamp, or deletion reason.
