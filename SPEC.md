...
## Account-first onboarding and suggestions

The public landing has one conversion action per section, “Organize my saves,” and every conversion enters `/app`. A new personal library requires Google sign-in; sample exploration remains available without an account. Existing device-only libraries are grandfathered and show an explicit backup, sign-in, and restore migration path. Device data is never silently attached to an account. When cloud configuration or new-library admission is unavailable, import is blocked honestly rather than falling back to a new anonymous library.

An authenticated empty dashboard shows the current Instagram Accounts Center export path and requires `saved_posts.json`, All time, and JSON. After import, smart organization starts only after the user accepts the disclosed first download and open-browser requirement. Draft groups appear as Suggested Collections on the dashboard and at `/app/suggested/:id`; each can be searched, lightly edited, and saved independently. Global search supports `scope=all|saved|suggested`, deduplicates saves, and labels memberships. Drafts remain owner-keyed on that device and do not synchronize.
