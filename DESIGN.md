...
Conversion hierarchy is deliberately sparse: each landing section has at most one product CTA, and its copy is “Organize my saves.” Sample exploration moves to the app entry screen. The closing composition is a three-part export-to-library path, using paper/file and collection-card objects rather than a decorative starburst.

The application entry screen is calm and account-focused. Empty accounts pair a concise import prompt with a permanently visible numbered export guide. Suggested collection pages default to reading and searching: a header pencil reveals name/description fields, while each save has a compact action menu. Suggested and saved surfaces remain visually related but suggestions always carry a text label; color alone never communicates status.
